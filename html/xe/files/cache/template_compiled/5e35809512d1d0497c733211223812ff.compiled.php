...
if($__Context->key == 'php_version'){ ?>(Ver. <?php echo $__Context->phpversion ?>)<?php } ?></strong>
				...
				<p><?php echo sprintf($__Context->lang->install_checklist_desc[$__Context->key], __XE_MIN_PHP_VERSION__) ?></p>
				<?php if($__Context->key == 'php_version' && $__Context->phpversion_warning === true){ ?><p>
					<?php echo sprintf($__Context->lang->install_checklist_desc['php_version_warning'], __XE_RECOMMEND_PHP_VERSION__) ?> <a href="http://php.net/supported-versions.php" target="_blank">[<?php echo $__Context->lang->more ?>]</a>
				</p><?php } ?>
			</li><?php }} ?>
		</ul>
